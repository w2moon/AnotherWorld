...
wl.equipment.prototype = {
    ...
    copy: function () {
        return new wl.equipment(wl.copy(this.dbobj));
    },
    ...
    addExp: function (v) {
        var pro = []
        pro.push({ level: this.dbobj.level, startexp: this.dbobj.exp, maxexp: this.getMaxExp(), endexp: this.dbobj.exp + v, hp: this.getProperty("MaxHP"), attack: this.getProperty("Attack"), defense: this.getProperty("Defense"), heal: this.getProperty("Heal") });
        ...
        while (this.dbobj.exp >= this.getMaxExp() && this.dbobj.level < this.getMaxLevel()) {
            ...
            if (this.dbobj.level == MAX_ROLE_LEVEL) {
                pro.push({ level: this.dbobj.level, startexp: 0, maxexp: 0, endexp: 0, hp: this.getProperty("MaxHP"), attack: this.getProperty("Attack"), defense: this.getProperty("Defense"), heal: this.getProperty("Heal") });
            }
            else {
                pro.push({ level: this.dbobj.level, startexp: 0, maxexp: this.getMaxExp(), endexp: this.dbobj.exp, hp: this.getProperty("MaxHP"), attack: this.getProperty("Attack"), defense: this.getProperty("Defense"), heal: this.getProperty("Heal") });
            }
        }
